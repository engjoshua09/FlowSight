...
def test_call_delta_atm():
    ...
    N(0.35) = 0.6368
    """
    result = compute_greeks(S=S, K=K, T=T, r=r, sigma=SIGMA, option_type="call")
    assert (
        abs(result["delta"] - 0.6368) < TOLERANCE
...
def test_put_delta_atm():
    ...
    """
    result = compute_greeks(S=S, K=K, T=T, r=r, sigma=SIGMA, option_type="put")
    assert (
        abs(result["delta"] - (-0.3632)) < TOLERANCE
...
def test_deep_itm_call_delta_near_one():
    ...
    result = compute_greeks(
        S=200.0,
        K=100.0,  # stock is 2x the strike = deep ITM
        T=T,
        r=r,
        sigma=SIGMA,
        option_type="call",
...
def test_deep_otm_call_delta_near_zero():
    ...
    result = compute_greeks(
        S=50.0,
        K=100.0,  # stock is half the strike = deep OTM
        T=T,
        r=r,
        sigma=SIGMA,
        option_type="call",
