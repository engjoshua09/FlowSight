...
export default function GreeksPanel({ initialSpot = 100 }) {
  ...
  const moneyness =
    spot > strike * 1.02 ? "ITM" :
    spot < strike * 0.98 ? "OTM" : "ATM";

  const moneynessColor =
    moneyness === "ITM" ? "#00d4aa" :
    moneyness === "OTM" ? "#ff6b6b" : "#f59e0b";
  ...
  return (
    <div style={{ display: "flex", gap: "1.5rem", flexWrap: "wrap", alignItems: "flex-start" }}>
      ...
      <div style={{
        ...
        <p style={{ color: "#666", fontSize: "0.78rem", marginBottom: "1.25rem", lineHeight: 1.5 }}>
          Adjust inputs to see how Greeks change in real time.
          Uses the same Black-Scholes model as the backend.
        </p>
        ...
        <SliderField
          ...
          value={strike} setValue={setStrike}
          min={10} max={1000} step={1}
          display={`$${strike}`}
          color="#fff"
          badge={moneyness} badgeColor={moneynessColor}
        />
        <SliderField
          label="Implied Volatility (IV)"
          value={iv} setValue={setIv}
      ...
      <div style={{ flex: "1 1 280px", minWidth: 260 }}>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0.75rem", marginBottom: "0.75rem" }}>
          <GreekCard
            symbol="Δ" name="Delta" value={greeks.delta} color="#a78bfa"
            desc={optType === "call"
              ? "Prob. of expiring ITM / $ move per $1 stock rise"
              : "$ move per $1 stock rise (negative for puts)"}
          />
          <GreekCard
            symbol="Γ" name="Gamma" value={greeks.gamma} color="#60a5fa"
            desc="Rate of delta change per $1 stock move"
          />
          <GreekCard
            symbol="Θ" name="Theta" value={greeks.theta} color="#f87171"
            desc="Value lost per calendar day (time decay)"
          />
          <GreekCard
            symbol="V" name="Vega" value={greeks.vega} color="#34d399"
        ...
        <div style={{
          ...
          <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "0.4rem" }}>
            ...
            <span style={{ color: "#a78bfa", fontSize: "0.85rem", fontWeight: "bold" }}>
              {greeks.delta}
            </span>
          </div>
        ...
        <div style={{
          marginTop: "0.75rem",
          background: "#1a1a1a", border: `1px solid ${moneynessColor}33`,
          borderRadius: "8px", padding: "0.75rem 1rem",
          display: "flex", alignItems: "center", justifyContent: "space-between",
        }}>
          <span style={{ color: "#666", fontSize: "0.8rem" }}>Moneyness</span>
          <span style={{
            color: moneynessColor, fontWeight: "bold", fontSize: "0.9rem",
            background: `${moneynessColor}18`, padding: "0.2rem 0.6rem",
            borderRadius: "4px",
          }}>
            {moneyness} —{" "}
            {optType === "call"
              ? (moneyness === "ITM" ? "Stock above strike" : moneyness === "OTM" ? "Stock below strike" : "Stock near strike")
              : (moneyness === "ITM" ? "Stock below strike" : moneyness === "OTM" ? "Stock above strike" : "Stock near strike")
            }
          </span>
        </div>
      </div>
...
function SliderField({ label, value, setValue, min, max, step, display, color, badge, badgeColor }) {
  return (
    <div style={{ marginBottom: "1.1rem" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "0.35rem" }}>
        ...
        <div style={{ display: "flex", gap: "0.4rem", alignItems: "center" }}>
          ...
          <span style={{ color, fontWeight: "bold", fontSize: "0.95rem", minWidth: "3.5rem", textAlign: "right" }}>
            {display}
          </span>
        </div>
      </div>
...
function GreekCard({ symbol, name, value, color, desc }) {
  return (
    <div style={{
      ...
      <div style={{ color: "#555", fontSize: "0.7rem", marginTop: "0.35rem", lineHeight: 1.4 }}>
        {desc}
      </div>
    </div>
  );
